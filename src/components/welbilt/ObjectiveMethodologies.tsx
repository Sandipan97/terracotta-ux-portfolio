      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          <Accordion type="single" defaultValue="objective" collapsible className="space-y-6">
            ...
            <AccordionItem value="methodologies" className="border-border rounded-2xl bg-card">
              <AccordionTrigger className="px-8 py-6 hover:no-underline">
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    <h2 className="font-heading text-2xl md:text-3xl font-bold text-foreground" data-lovable-editable="welbilt-methodology-title">Research Methodologies</h2>
                    <p className="text-muted-foreground mt-1" data-lovable-editable="welbilt-methodology-subtitle">Systematic approach to understanding user needs</p>
                  </div>
                </div>
              </AccordionTrigger>
              <AccordionContent className="px-8 pb-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {methodologies.map((method, index) => (
                    ...
                    >
                       <Card className="bg-muted border-border hover:border-blue-300 transition-all duration-300 h-full">
                         <CardContent className="p-6">
                           <div className="flex items-start gap-4">
                             ...
                             <div>
                                <h3 className="font-semibold text-foreground text-lg mb-2" data-lovable-editable={`welbilt-method-${index}-title`}>{method.title}</h3>
                                <p className="text-muted-foreground text-sm leading-relaxed" data-lovable-editable={`welbilt-method-${index}-description`}>{method.description}</p>
                                <div className="mt-3 flex items-center gap-2">
                                  <div className="w-2 h-2 bg-blue-600 rounded-full"></div>
                                  <span className="text-xs text-muted-foreground" data-lovable-editable={`welbilt-method-${index}-connection`}>Connects to Research Question {index + 1}</span>
                                </div>
                             </div>
                           </div>
                         </CardContent>
