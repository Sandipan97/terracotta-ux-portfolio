...
float smin( float a, float b, float k ) {
    k *= 8.4; // Increased from 6.0 by 40% for more blur separation
...
float sdf(vec3 p) {
    ...
    final = smin(final, nextSphere, 0.14); // Increased from 0.1 by 40%
...
void main() {
    ...
    // Dark Terracotta color from Bengali design system (#A50026)
    vec3 terracottaColor = vec3(0.647, 0.0, 0.149); // #A50026 converted to RGB
    ...
    if (t > 0.0) {
        ...
        vec3 color = terracottaColor * fresnel;
        gl_FragColor = vec4(color, 1.0);
    } else {
        gl_FragColor = vec4(terracottaColor * 0.1, 1.0); // Subtle background tint
    }
}
`;
